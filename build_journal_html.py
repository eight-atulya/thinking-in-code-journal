...
def render_markdown(md: str) -> tuple[str, list[dict[str, str]]]:
    ...
    while i < len(lines):
        ...
        if stripped.startswith("```"):
            ...
            i += 1
            out.append(
                f"<figure class=\"code-card\"><div class=\"code-card-head\"><figcaption>{html.escape(lang)}</figcaption>"
                f"<button class=\"copy-code\" type=\"button\">Copy</button></div>"
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .search-panel {{
      display: grid; gap: 8px; padding: 10px; border-radius: var(--radius);
      background: linear-gradient(180deg, rgba(255,255,255,.045), rgba(255,255,255,.018));
    }}
    .search-panel label {{
      color: var(--white); font-size: .72rem; font-weight: 900; letter-spacing: .1em; text-transform: uppercase;
    }}
    .search-box {{
      display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 6px;
      min-height: 44px; border-radius: var(--radius); background: var(--panel);
      box-shadow: inset 0 -1px 0 var(--line);
    }}
    input[type="search"] {{
      width: 100%; min-width: 0; border: 0; border-radius: var(--radius);
      background: transparent; color: var(--text); padding: 12px; outline: none; font: inherit;
    }}
    input[type="search"]:focus {{ box-shadow: inset 0 -3px 0 var(--yellow); }}
    .search-clear {{
      display: none; align-items: center; justify-content: center; min-width: 36px; min-height: 36px;
      margin-right: 4px; padding: 0 9px; border: 0; border-radius: 6px; background: rgba(255,255,255,.06);
      color: var(--muted); font-size: .74rem; font-weight: 900;
    }}
    .search-clear.visible {{ display: inline-flex; }}
    .search-clear:hover {{ color: var(--yellow); background: rgba(255,210,31,.1); }}
    .search-meta {{ color: var(--muted); font-size: .76rem; line-height: 1.35; min-height: 1.35em; }}
    .search-results {{ display: grid; gap: 6px; max-height: 280px; overflow: auto; padding-right: 2px; }}
    .search-result {{
      display: grid; gap: 3px; padding: 8px 9px; border-radius: var(--radius);
      color: var(--text); text-decoration: none; background: rgba(255,255,255,.035);
    }}
    .search-result:hover, .search-result:focus-visible {{
      outline: none; color: var(--white); background: rgba(255,210,31,.11);
    }}
    .search-result strong {{
      min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
      color: var(--white); font-size: .84rem; line-height: 1.25;
    }}
    .search-result span {{ color: var(--muted); font-size: .74rem; line-height: 1.32; }}
    .toc-link.search-muted {{ display: none; }}
    .button-row {{ display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }}
    button, .source-link {{
      border: 1px solid var(--line); border-radius: var(--radius); background: var(--panel-2);
      color: var(--text); padding: 9px 10px; cursor: pointer; font: inherit; text-decoration: none;
    }}
    ...
    .pitch-link {{
      ...
      background: linear-gradient(135deg, rgba(255,210,31,.16), rgba(255,45,45,.08));
    ...
    .toc-head {{
      position: sticky; top: -22px; z-index: 3; display: flex; align-items: center; justify-content: space-between;
      gap: 10px; margin: 18px -4px 10px; padding: 10px 4px 8px;
      background: linear-gradient(180deg, rgba(10,10,10,.98), rgba(10,10,10,.82));
      border-bottom: 1px solid rgba(255,255,255,.08);
    }}
    body.light .toc-head {{ background: linear-gradient(180deg, rgba(255,253,246,.98), rgba(255,253,246,.82)); }}
    .toc-head span {{ color: var(--white); font-size: .76rem; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; }}
    .toc-head b {{
      color: var(--yellow); font-size: .72rem; font-weight: 900; padding: 3px 7px; border: 1px solid rgba(255,210,31,.25);
      border-radius: 999px; background: rgba(255,210,31,.08);
    }}
    nav {{ display: grid; gap: 5px; padding-bottom: 36px; }}
    .toc-link {{
      position: relative; display: grid; grid-template-columns: auto minmax(0, 1fr); align-items: start; gap: 9px;
      color: var(--muted); text-decoration: none; min-height: 36px; padding: 8px 9px;
      border: 0; border-radius: var(--radius); font-size: .86rem; line-height: 1.25;
    }}
    .toc-link:hover {{ color: var(--white); background: rgba(255,255,255,.035); }}
    .toc-kicker {{
      display: inline-flex; align-items: center; justify-content: center; min-height: 20px; padding: 0 7px;
      color: var(--muted); border: 1px solid var(--line); border-radius: 999px; font-size: .62rem;
      font-weight: 900; letter-spacing: .04em; text-transform: uppercase; white-space: nowrap; background: rgba(255,255,255,.025);
    }}
    .toc-title {{ min-width: 0; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }}
    .toc-link[data-level="1"] {{
      grid-template-columns: 1fr; gap: 4px; margin: 8px 0 2px; padding: 12px 12px 12px 14px;
      color: var(--text); font-weight: 900; background: linear-gradient(135deg, rgba(255,45,45,.13), rgba(255,210,31,.04));
      box-shadow: none;
    }}
    .toc-link[data-level="1"] .toc-kicker {{ width: max-content; color: var(--yellow); border-color: rgba(255,210,31,.28); padding: 2px 7px; }}
    .toc-link[data-level="1"] .toc-title {{ font-size: .96rem; line-height: 1.22; }}
    .toc-link[data-level="2"] {{ margin-left: 8px; color: color-mix(in srgb, var(--text) 78%, var(--muted)); font-weight: 750; }}
    .toc-link[data-level="2"] .toc-kicker {{ color: var(--yellow); border-color: rgba(255,210,31,.2); }}
    .toc-link[data-level="3"] {{ margin-left: 20px; grid-template-columns: auto minmax(0, 1fr); font-size: .78rem; min-height: 31px; padding-block: 6px; }}
    .toc-link[data-level="3"] .toc-kicker {{ font-size: .56rem; min-height: 18px; color: var(--muted); }}
    .toc-link.active {{
      color: var(--white);
      background: linear-gradient(135deg, rgba(255,210,31,.13), rgba(255,45,45,.08));
      box-shadow: 0 10px 26px rgba(0,0,0,.14);
    }}
    .toc-link.active .toc-kicker {{ color: #000; background: var(--yellow); border-color: var(--yellow); }}
    main {{ min-width: 0; }}
    .hero {{
    ...
    @media (max-width: 900px) {{
      ...
      aside:not(.mobile-open) .toc-head,
      aside:not(.mobile-open) nav {{ display: none; }}
      aside.mobile-open .controls {{ margin-top: 14px; }}
      .hero {{ min-height: 64vh; padding-top: 54px; overflow-x: clip; }}
      .hero-content {{ min-width: 0; max-width: 100%; }}
      .hero h1 {{ font-size: clamp(2.8rem, 11vw, 4.5rem); line-height: .96; overflow-wrap: anywhere; max-width: 100%; }}
      .hero p {{ font-size: 1rem; }}
      .kicker {{ overflow-wrap: anywhere; }}
      .signal-strip {{ gap: 8px; margin-top: 22px; max-width: 100%; overflow: hidden; }}
    ...
    @media (max-width: 430px) {{
      .hero {{ padding-inline: 24px; overflow-x: clip; }}
      .hero-content {{ min-width: 0; max-width: 100%; }}
      .hero h1 {{ font-size: 2.8rem; line-height: .96; overflow-wrap: anywhere; max-width: 100%; }}
      .hero p {{ max-width: 100%; }}
      .kicker {{ font-size: .64rem; line-height: 1.35; overflow-wrap: anywhere; }}
      .signal-strip {{ max-width: 100%; overflow: hidden; }}
      .signal-strip span {{ min-width: 0; max-width: 100%; }}
...
<body>
  ...
  <div class="layout">
    <aside>
      ...
      <div class="controls">
        <div class="search-panel" role="search" aria-label="Search journal">
          <label for="search">Find in journal</label>
          <div class="search-box">
            <input id="search" type="search" placeholder="Search concepts..." autocomplete="off" spellcheck="false" aria-describedby="searchMeta">
            <button id="searchClear" class="search-clear" type="button" aria-label="Clear search">Clear</button>
          </div>
          <div id="searchMeta" class="search-meta" aria-live="polite">Search headings, lesson text, code, and checks.</div>
        ...
        <div class="button-row">
          ...
          <a class="source-link pitch-link" href="if-may-pitch.html" target="_blank" rel="noopener">Pitch</a>
        </div>
        <div class="button-row">
      ...
      <div class="toc-head"><span>Journal map</span><b id="tocCount"></b></div>
  ...
  <script>
    ...
    const tocLevelLabel = level => level === '1' ? 'Part' : level === '2' ? 'Entry' : 'Step';
    toc.innerHTML = tocData.map((item, index) => `
      <a class="toc-link" href="#${{item.id}}" data-level="${{item.level}}" data-toc-index="${{index}}">
        <span class="toc-kicker">${{tocLevelLabel(item.level)}}</span>
        <span class="toc-title">${{item.title}}</span>
      </a>`).join('');
    document.getElementById('tocCount').textContent = `${{tocData.length}} links`;
    ...
    const allSections = [...document.querySelectorAll('.entry-section')];
    const linkById = new Map(links.map(link => [decodeURIComponent(link.getAttribute('href').slice(1)), link]));
    const tocById = new Map(tocData.map(item => [item.id, item]));
    ...
    const searchIndex = allSections.map((section, index) => {{
      const heading = section.querySelector('h1, h2, h3')?.innerText.trim() || `Section ${{index + 1}}`;
      const rawText = section.innerText.replace(/\\s+/g, ' ').trim();
      const tocItem = tocById.get(section.id) || {{}};
      return {{
        section,
        id: section.id,
        href: `#${{encodeURIComponent(section.id)}}`,
        title: heading,
        level: tocLevelLabel(tocItem.level || '3'),
        rawText,
        searchable: normalizeSearchText(`${{heading}} ${{rawText}}`),
        titleSearchable: normalizeSearchText(heading),
        link: linkById.get(section.id)
      }};
    }});

    let dueOnly = false;
    let lastSearchMatches = new Set(searchIndex.map(item => item.id));
    ...
    function updateVisibility() {{
      allSections.forEach(section => {{
        const blockedByDue = dueOnly && section.dataset.due !== 'true';
        const blockedBySearch = !lastSearchMatches.has(section.id);
        section.classList.toggle('hidden', blockedByDue || blockedBySearch);
      }});
      links.forEach(link => {{
        const id = decodeURIComponent(link.getAttribute('href').slice(1));
        link.classList.toggle('search-muted', !lastSearchMatches.has(id));
      }});
    }}
    ...
    function makeSnippet(text, tokens) {{
      ...
      const firstIndex = tokens.reduce((best, token) => {{
        ...
        return found === -1 ? best : Math.min(best, found);
      }}, Number.POSITIVE_INFINITY);
      const start = Number.isFinite(firstIndex) ? Math.max(0, firstIndex - 48) : 0;
      const prefix = start > 0 ? '...' : '';
      const suffix = start + 132 < text.length ? '...' : '';
      let snippet = escapeHtml(prefix + text.slice(start, start + 132).trim() + suffix);
      tokens.forEach(token => {{
        snippet = snippet.replace(new RegExp(`(${{escapeRegExp(escapeHtml(token))}})`, 'ig'), '<mark>$1</mark>');
      }});
      return snippet;
    }}

    function scoreSearchItem(item, tokens, phrase) {{
      if (!tokens.every(token => item.searchable.includes(token))) return 0;
      let score = 10;
      if (item.titleSearchable.includes(phrase)) score += 24;
      if (item.searchable.includes(phrase)) score += 10;
      tokens.forEach(token => {{
        if (item.titleSearchable.includes(token)) score += 8;
        if (item.searchable.startsWith(token)) score += 4;
      }});
      return score;
    }}

    function applySearch() {{
      const phrase = normalizeSearchText(search.value);
      const tokens = phrase.split(' ').filter(Boolean);
      const active = tokens.length > 0;
      searchClear.classList.toggle('visible', active);

      if (!active) {{
        lastSearchMatches = new Set(searchIndex.map(item => item.id));
        searchMeta.textContent = 'Search headings, lesson text, code, and checks.';
      ...
      const matches = searchIndex
        .map(item => ({{ ...item, score: scoreSearchItem(item, tokens, phrase) }}))
        .filter(item => item.score > 0)
        .sort((a, b) => b.score - a.score || a.title.localeCompare(b.title));
      lastSearchMatches = new Set(matches.map(item => item.id));
      updateVisibility();

      const visibleMatches = matches.filter(item => !dueOnly || item.section.dataset.due === 'true');
      searchMeta.textContent = visibleMatches.length
        ? `${{visibleMatches.length}} match${{visibleMatches.length === 1 ? '' : 'es'}}. Top results are ready below.`
        : dueOnly
          ? 'No due lessons match this search.'
          : 'No match found. Try a concept, command, or heading.';

      searchResults.hidden = visibleMatches.length === 0;
      searchResults.innerHTML = visibleMatches.slice(0, 8).map(item => `
        <a class="search-result" href="${{item.href}}" data-search-jump>
          <strong>${{escapeHtml(item.title)}}</strong>
          <span>${{item.level}} · ${{makeSnippet(item.rawText, tokens)}}</span>
        </a>
      `).join('');

      visibleMatches.slice(0, 10).forEach(item => setSectionCollapsed(item.section, false));
    }}
    ...
    document.getElementById('expandAll').addEventListener('click', e => {{
      ...
      dueOnly = false;
      document.getElementById('dueFilter').textContent = 'Due only';
      search.value = '';
      applySearch();
      allSections.forEach(section => {{
    ...
    search.addEventListener('keydown', event => {{
      ...
      if (event.key === 'Enter') {{
        const firstResult = searchResults.querySelector('[data-search-jump]');
    ...
    searchResults.addEventListener('click', event => {{
      const link = event.target.closest('[data-search-jump]');
      if (!link) return;
      openSectionFromLink(link.getAttribute('href'));
    }});
